...
- Sunucu bir **üst küme** çeker: son **48 saatte ne varsa** (`ITEM_MAX_AGE_HOURS`); kaynak başına yapay sınır yok (patolojik bir feed'e karşı yalnızca genel `MAX_ITEMS = 300` güvenlik tavanı) — `src/lib/config.ts`.
...
Anasayfanın üstündeki çubuktan arama yapılır; yanındaki **Filtreler** panelinden kaynak, pencere ve kaynak başına sayı seçilir. Hepsi **tarayıcıda** çalışır: sunucu bir kez üst kümeyi üretir (48 saatte ne varsa, pratikte ~130 kalem), arama/filtre sunucuya istek atmadan anında uygulanır.
