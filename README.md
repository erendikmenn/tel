...
- **Dizgi:** üst bant (3 kısa teaser) → **gotik künye** → künye şeridi (gün · tarih · sayı) → manşet + italik deck → **fotoğrafın SOLUNA ve SAĞINA iki ayrı haber** (flanker) → gerekçeli 3 kolon manşet gövdesi (ilk harf büyük) → 3 fotoğraflı haber + çerçeveli **Öne çıkanlar** sütunu → 4 kolon **kısa kısa** (kalan yüksekliği doldurur) → alt bilgi.
- **Baskı:** A4 210×297 mm, **tek sayfa** ve **sayfa dolu**. Yükseklik içerikten gelir (kırpma yok); punto ve fotoğraflar sayfayı dolduracak şekilde ölçeklendi — yazdırma yüksekliği ölçüldü: **273 mm** (A4 metin alanı 283 mm). Manşet künyesi 58 px, banner 40 px, gövde 11.6 px, manşet fotoğrafı 60 mm, haber fotoğrafı 29 mm. Fotoğraflar **renkli** (`print-color-adjust: exact`).
- **PDF indir (tek tık):** sağ üstteki **PDF indir** düğmesi `/gazete/pdf` rotasına gider; rota sunucuda **makinedeki Chrome/Chromium** ile sayfayı PDF'e basar (`--no-pdf-header-footer`) ve indirir. Yazdırma penceresi açılmadığı için kenar boşluğunda **`localhost` satırı çıkmaz**. Tarayıcı yolu: `CHROME_PATH` ortam değişkeni → yoksa macOS uygulama yolları → yoksa Playwright önbelleği. Makinede tarayıcı yoksa rota açıklayıcı bir mesaj döner ve **Yazdır** düğmesi (tarayıcı yazdırma penceresi) yedek olarak kalır.
