...
- **PDF indir (tek tık):** **PDF indir** düğmesi `/gazete/pdf` rotasına gider. Rota Chrome'u **CDP ile** açar, sayfayı yazdırma görünümünde ölçer ve `Page.printToPDF` ile **tam sayfaya ölçekleyerek** basar: `ölçek = 281.5 mm / doğal yükseklik`. Böylece içerik kısa günde büyütülür, uzun günde küçültülür — **her zaman tek sayfa, kenara kadar dolu, hiçbir satır kesilmez**. Yazdırma penceresi açılmadığı için kenar boşluğunda `localhost` çıkmaz. Ölçüm **196 mm** genişlikte yapılır (yazdırma alanı); 210 mm ile ölçmek kolonları geniş gösterip yüksekliği ~22 mm eksik hesaplıyordu. Tarayıcı yolu: `CHROME_PATH` → macOS uygulama yolları → Playwright önbelleği; bulunamazsa rota açıklama döner, **Yazdır** yedek kalır.
