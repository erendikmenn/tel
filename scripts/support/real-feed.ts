...
import { FEED_ITEM_LIMIT, ITEM_MAX_AGE_HOURS } from "../../src/lib/config";
...
/** Canlı 5 RSS'ten gerçek kalemleri çeker (uygulamayla aynı: 36 saat, 12/feed, tekilleştirme). */
export async function fetchRealItems(): Promise<DigestItem[]> {
  ...
  for (const result of results) {
    ...
    for (const raw of parsed.items as RawItem[]) {
      ...
      count += 1;
      if (count >= FEED_ITEM_LIMIT) break;
    }
  }
